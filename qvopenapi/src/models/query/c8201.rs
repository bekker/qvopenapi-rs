...
pub fn parse_c8201_response1_array(block_data: *const c_char, block_len: i32) -> Result<Value, QvOpenApiError> {
	unsafe {
        ...
        let ret: Result<Vec<C8201Response1>, QvOpenApiError> = res.iter()
            .map(parse_c8201_response1)
            .collect();
		Ok(json!(ret?))
	}
}
